...
sm_slots:save("hero", { name = "Hero" })
sm_slots:save("save2", { name = "Save2" })

local info = sm_slots:slot_info("hero")
assert(info.exists, "hero slot should exist")
assert(info.size > 0, "size should be positive")
log("  slot_info('hero'): exists=%s, size=%d, version=%s", info.exists, info.size, tostring(info.save_version))

local exists = sm_slots:slot_exists("hero")
assert(exists == true)
local exists_fake = sm_slots:slot_exists("nope")
assert(exists_fake == false)
log("  slot_exists('hero')=%s, slot_exists('nope')=%s", exists, exists_fake)
...
-- Delete
ok, err = sm_slots:delete("save2")
assert(ok, "Delete: " .. tostring(err))
assert(not sm_slots:slot_exists("save2"))
log("  delete('save2'): OK")
...
local tampered = fs.read(safe_path):gsub("42", "99")
